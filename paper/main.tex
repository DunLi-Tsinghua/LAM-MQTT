...
\begin{figure*}[!t]
  ...
  \caption{MQTT-based IoT traffic observability under encrypted broker-mediated monitoring. IoT publishers and subscribers communicate with an MQTT broker through TLS-protected PUBLISH/SUBSCRIBE flows; payloads, raw topics, credentials, client identifiers, raw IP identities, and exact timestamps are excluded, while broker-side flow metadata remains observable.}
...
\begin{figure*}[!t]
  ...
  \caption{LAM-MQTT pipeline for MQTT-based IoT traffic. Packet traces, biflow features, and flow-window metadata are passed through MQTT-aware leakage audit, observability feature policies, baseline IDS models, and within-dataset, MQTT-to-MQTT, and external IoT robustness evaluations.}
  \label{fig:framework}
\end{figure*}
...
\begin{figure*}[!t]
  ...
  \caption{MQTT/TLS observability policies for metadata-only IDS. Raw MQTT application and identity fields are excluded, while full audited, broker-side secure, and strict encrypted-flow views retain progressively constrained flow metadata available near the MQTT broker.}
  \label{fig:views}
\end{figure*}
