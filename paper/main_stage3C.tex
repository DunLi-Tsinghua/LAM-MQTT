...
Machine-learning-based IoT intrusion detection has been studied extensively because IoT deployments combine constrained devices, heterogeneous protocols, and exposed network services \cite{algaradi2020survey,hussain2020machine,hodo2017taxonomy,buczak2016survey,khraisat2019survey}. MQTT is a particularly important protocol in this context. Its broker-mediated publish-subscribe model is efficient, but weak authentication, insecure deployment choices, broker exposure, and abnormal publish/subscribe behavior can produce detectable network-level symptoms. MQTT-IoT-IDS2020 introduced an MQTT-focused case study and evaluated packet-, uniflow-, and biflow-level features for attack detection \cite{hindy2020mqttiot}. MQTTset provided another MQTT benchmark for machine-learning experiments \cite{vaccari2020mqttset}. MQTTEEB-D extends this line of work with a real-world MQTT cybersecurity dataset collected from an IoT deployment \cite{mqtteebd2025}. These datasets make MQTT attack detection experimentally tractable, but their use still requires careful feature auditing.

Several broader IoT and IIoT datasets provide additional context. IoT-23 contains malware and benign IoT network traffic and is useful for external IoT robustness evaluation \cite{iot23}. CICIoT2023, ToN-IoT, Bot-IoT, N-BaIoT, and Edge-IIoTset cover different IoT and IIoT attack surfaces, device types, and collection environments \cite{ciciot2023,alsaedi2020toniot,koroniotis2019botiot,meidan2018nbaiot,ferrag2022edgeiiotset}. IoT botnet studies, including Mirai analyses, further show that compromised devices can generate large-scale network effects that are visible in timing, volume, and connection behavior \cite{antonakakis2017mirai,kolias2017mirai}. These corpora support benchmarking, but they also differ substantially in capture setup, labels, feature definitions, and attack distributions.

\subsection{Encrypted Traffic Analysis and Metadata-Only Monitoring}
Encrypted traffic analysis has long used timing, direction, and size metadata when payload inspection is unavailable. Classical traffic-classification work used host behavior, Bayesian models, and flow statistics to infer application classes without relying only on port numbers \cite{karagiannis2005blinc,moore2005bayesian}. Later studies showed that time-related features can characterize encrypted and VPN traffic \cite{drapergil2016vpn}. Mobile and encrypted-application fingerprinting work further demonstrated that packet sizes and timing can reveal behavioral structure even when content is protected \cite{conti2016android,taylor2016appscanner}. Deep learning studies later explored packet-sequence and representation-learning approaches for encrypted traffic classification \cite{lotfollahi2020deeppacket,rezaei2019overview,aceto2019mimetic,wang2017encryptedcnn}. IoT traffic classifiers also use convolutional and recurrent models over flow or packet representations \cite{lopezmartin2017traffic,sivanathan2019iotdevices}.
...
Network IDS evaluation is vulnerable to closed-world assumptions. Sommer and Paxson emphasized the gap between machine-learning success in controlled settings and operational network intrusion detection \cite{sommer2010outside}. Broader security-ML critiques have also shown that leakage, unrealistic splits, and hidden experimental bias can inflate reported performance \cite{arp2022dosdonts,pendlebury2019tesseract}. Dataset surveys similarly show that benchmark suitability depends on capture environment, labeling, feature construction, attack diversity, and temporal realism \cite{ring2019survey}. More generally, dataset shift and transfer-learning studies show that training and test distributions often differ in real deployments \cite{quionero2009datasetshift,pan2010transfer}.
...
Public security datasets have shaped IDS evaluation, including CICIDS2017 and UNSW-NB15 for general network intrusion detection \cite{sharafaldin2018cicids,moustafa2015unsw}, MQTT-IoT-IDS2020, MQTTset, and MQTTEEB-D for MQTT-specific evaluation \cite{hindy2020mqttiot,vaccari2020mqttset,mqtteebd2025}, and IoT-23, ToN-IoT, Bot-IoT, N-BaIoT, CICIoT2023, and Edge-IIoTset for broader IoT and IIoT scenarios \cite{iot23,ciciot2023,alsaedi2020toniot,koroniotis2019botiot,meidan2018nbaiot,ferrag2022edgeiiotset}. Flow-based IDS surveys and online anomaly detectors such as Kitsune reinforce the practical value of compact network metadata for high-speed or resource-constrained monitoring \cite{sperotto2010flow,umer2017flow,garcia2009anomaly,ahmed2016survey,mirsky2018kitsune}. These datasets and methods are complementary rather than interchangeable. In particular, IoT-23 should be interpreted as an external IoT robustness dataset in this work, not as an MQTT benchmark.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{Fig2_framework.pdf}
  \caption{LAM-MQTT evaluation framework. The pipeline ingests public datasets, applies leakage audit, builds feature policies, trains baseline models, and evaluates within-dataset, cross-dataset, and external robustness settings.}
  \label{fig:framework}
\end{figure}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{Fig3_crypto_observability_views.pdf}
  \caption{Cryptographic observability views. Full audited, broker-side secure, and strict encrypted-flow metadata are separated from fields excluded by the leakage audit.}
  \label{fig:views}
\end{figure}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{Fig4_within_dataset_main_macro_f1.pdf}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{Fig5_crypto_observability_ablation_revised.pdf}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{Fig6_mqtt_to_mqtt_cross_dataset_main.pdf}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{Fig7_feature_family_ablation.pdf}
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\linewidth]{Fig8_feature_importance.pdf}
...
The study uses public datasets: MQTTEEB-D, MQTT-IoT-IDS2020, and IoT-23. The implementation records feature views, leakage audit decisions, processed metrics, and figures for reproducibility. The code and processed metadata tables can be released as supplementary materials or upon acceptance, subject to dataset redistribution terms. The method does not decrypt TLS traffic and does not replace authentication, access control, or protocol hardening. Sensitive raw fields such as payloads, topics, credentials, client identifiers, raw IP identities, and exact timestamps are excluded from model inputs.

\bibliographystyle{IEEEtran}
\bibliography{references_stage3C}
\end{document}
